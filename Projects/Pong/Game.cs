...
public class Game {
	...
	public Points Run(){
		...
		ballStopwatch.Start();
		ballTimer = new Timer(o => do_ball(o), null, 0, Opts.ball_delay);
		ballIsRunning = true;
	while(Points.Min() > 0) {
		int react;
		...
		if(ballIsRunning && opponentStopwatch.IsRunning && opponentStopwatch.Elapsed > opponentInputDelay){
